...
public interface ActifRepository extends JpaRepository<Actif, Integer> {
        ...
        @Query("SELECT a from Actif a WHERE a.collaborateur = ?1 AND a.date >= ?2 ORDER BY a.date")
        List<Temps> findByAfterDate(Collaborateur collaborateur, Date from);


        @Query("SELECT a from Actif a WHERE a.collaborateur = ?1 AND a.date BETWEEN ?2 AND ?3 ORDER BY a.date")
